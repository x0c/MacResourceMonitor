...
- **【排坑 2026-09-19 · 名单顶上两行黑图标「0」各占约 17%】**这不是监视器主程序（主程序仍是下面那一行约 1%）。这是本应用为网络表拉起的按进程网速采样，覆盖安装 / 退出时父进程没了，采样被系统收养后空转约一核（活动监视器约 130%，整机口径约 17%），内存接近 0。人话名把 `-L 0` 的 `0` 当成了工具名。禁止再把半秒后的补杀放到应用退出之后才执行；退出必须同步停采并清掉无主残留；启动时也要清上一份留下的同类采样；活着的采样必须折进 Mac Resource Monitor，不得单独占行叫 `0`。现场：两个无主采样已跑约 27–28 分钟，当前监视器实例比它们更晚启动。
...
| 修复后台网络采样高占用 | `NetworkListModel` 的采样生命周期、`nettop` 子进程创建与退出、后台预热约束 | 只看应用本体 CPU、把短命子进程漏掉，或为降占用取消预热造成首开空白 | 面板收起和展开都不再每秒派生高耗能进程；首开仍立即显示最近一帧；持续观测应用及全部子进程的 CPU 后风扇不再被本应用触发 |
...
- 改网络采样实现或周期后，必须用实际安装版连续观察至少 60 秒，并同时核对应用本体和全部子进程：面板收起、网络面板展开、网速显示关闭三种状态都不得每秒新建高耗能 `nettop`；不能只看界面读数正确或单元测试通过。随后制造持续下载与空闲两种流量，确认首开有最近一帧、上下行方向正确、空闲名单保持规则未回退，并确认退出应用后无采样子进程残留。
